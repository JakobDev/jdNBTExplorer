...
def write_translation_status(project_root: str, appstream_path: str) -> None:
    ...
    translation_dir = os.path.join(project_root, "jdNBTExplorer", "translations")
    for translation_file in os.listdir(translation_dir):
        if not translation_file.endswith(".ts"):
            continue

        language = translation_file.removeprefix("jdNBTExplorer_").removesuffix(".ts")
    ...
    xml.etree.ElementTree.indent(appstream_tree)
...
def main() -> None:
    ...
    install_data_file(os.path.join(project_root, "jdNBTExplorer", "Logo.png"), os.path.join(args.prefix, "share", "icons", "hicolor", "256x256", "apps", "page.codeberg.JakobDev.jdNBTExplorer.png"))

    create_directory(os.path.join(args.prefix, "share", "applications"))
    create_directory(os.path.join(args.prefix, "share", "metainfo"))

    appstream_path = os.path.join(args.prefix, "share", "metainfo", "page.codeberg.JakobDev.jdNBTExplorer.metainfo.xml")

    subprocess.run(["msgfmt", "--desktop", "--template", os.path.join(project_root, "deploy", "page.codeberg.JakobDev.jdNBTExplorer.desktop"), "-d", os.path.join(project_root, "deploy", "translations"), "-o", os.path.join(args.prefix, "share", "applications", "page.codeberg.JakobDev.jdNBTExplorer.desktop")], check=True)
    subprocess.run(["msgfmt", "--xml", "--template", os.path.join(project_root, "deploy", "page.codeberg.JakobDev.jdNBTExplorer.metainfo.xml"), "-d", os.path.join(project_root, "deploy", "translations"), "-o", appstream_path], check=True)
